(() => {
    ...
    function windowLoad() {
        ...
        function counterAnimate(counter) {
            ...
            let countValue = counter.querySelector(".counter__value");
            const duration = parseInt(counter.dataset.counterDuration) ? parseInt(counter.dataset.counterDuration) : 1e3;
            const startValue = parseInt(countValue.innerHTML);
            const startPosition = 0;
            const step = timestamp => {
                if (!startTimestamp) startTimestamp = timestamp;
                const progress = Math.min((timestamp - startTimestamp) / duration, 1);
                countValue.innerHTML = Math.floor(progress * (startPosition + startValue));
                if (progress < 1) window.requestAnimationFrame(step); else {
                    let seperation = counter.querySelector(".counter__value._seperation");
                    if (seperation) seperation.innerHTML = seperation.innerHTML.replace(/(\d)(?=(\d{3})+$)/g, "$1,");
                }
            };
        ...
        let observer = new IntersectionObserver(((entries, observer) => {
            entries.forEach((entry => {
                if (entry.isIntersecting) {
                    ...
                    if (counterItems.length) counterInit(counterItems);
                    observer.unobserve(targetElement);
                }
            }));
        }), options);
